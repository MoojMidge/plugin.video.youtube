...
class AbstractSettings(object):
    ...
    def get_bool(self, setting, default=None, echo_level=1):
        raise NotImplementedError()

    def set_bool(self, setting, value, echo_level=1):
        raise NotImplementedError()

    def get_int(self, setting, default=-1, converter=None, echo_level=1):
        raise NotImplementedError()

    def set_int(self, setting, value, echo_level=1):
        raise NotImplementedError()

    def get_string(self, setting, default='', echo_level=1):
        raise NotImplementedError()

    def set_string(self, setting, value, echo_level=1):
        raise NotImplementedError()

    def get_string_list(self, setting, default=None, echo_level=1):
        raise NotImplementedError()

    def set_string_list(self, setting, value, echo_level=1):
        raise NotImplementedError()
    ...
    def items_per_page(self, value=None):
        if value is not None:
            return self.set_int(SETTINGS.ITEMS_PER_PAGE, value, echo_level=2)
        return self.get_int(SETTINGS.ITEMS_PER_PAGE, 50, echo_level=2)
    ...
    def fixed_video_quality(self, value=None):
        default = 3
        if value is None:
            _value = self.get_int(SETTINGS.VIDEO_QUALITY, default, echo_level=2)
        else:
            _value = value
        if _value not in self._VIDEO_QUALITY_MAP:
            _value = default
        if value is not None:
            self.set_int(SETTINGS.VIDEO_QUALITY, _value, echo_level=2)
        return self._VIDEO_QUALITY_MAP[_value]

    def ask_for_video_quality(self):
        if self.use_mpd_videos():
            return self.get_int(SETTINGS.MPD_STREAM_SELECT, echo_level=2) == 4
        return self.get_bool(SETTINGS.VIDEO_QUALITY_ASK, False, echo_level=2)

    def fanart_selection(self):
        return self.get_int(SETTINGS.FANART_SELECTION, 2, echo_level=2)

    def cache_size(self, value=None):
        if value is not None:
            return self.set_int(SETTINGS.CACHE_SIZE, value, echo_level=2)
        return self.get_int(SETTINGS.CACHE_SIZE, 20, echo_level=2)

    def get_search_history_size(self):
        return self.get_int(SETTINGS.SEARCH_SIZE, 10, echo_level=2)
    ...
    def setup_wizard_enabled(self, value=None):
        ...
        if value is False:
            self.set_int(SETTINGS.SETUP_WIZARD_RUNS, run_required, echo_level=2)
            return self.set_bool(SETTINGS.SETUP_WIZARD, False, echo_level=2)
        if value is True:
            self.set_int(SETTINGS.SETUP_WIZARD_RUNS, 0, echo_level=2)
            return self.set_bool(SETTINGS.SETUP_WIZARD, True, echo_level=2)

        last_run = self.get_int(SETTINGS.SETUP_WIZARD_RUNS, 0, echo_level=2)
        if last_run < run_required:
            self.set_int(SETTINGS.SETUP_WIZARD_RUNS, run_required, echo_level=2)
            self.set_bool(SETTINGS.SETTINGS_END, True, echo_level=2)
            return run_required
        return self.get_bool(SETTINGS.SETUP_WIZARD, False, echo_level=2)
    ...
    def use_isa(self, value=None):
        if value is not None:
            return self.set_bool(SETTINGS.USE_ISA, value, echo_level=2)
        return self.get_bool(SETTINGS.USE_ISA, False, echo_level=2)

    def subtitle_download(self):
        return self.get_bool(SETTINGS.SUBTITLE_DOWNLOAD, False, echo_level=2)

    def audio_only(self):
        return self.get_bool(SETTINGS.AUDIO_ONLY, False, echo_level=2)

    def get_subtitle_selection(self):
        return self.get_int(SETTINGS.SUBTITLE_SELECTION, 0, echo_level=2)

    def set_subtitle_selection(self, value):
        return self.set_int(SETTINGS.SUBTITLE_SELECTION, value, echo_level=2)

    def set_subtitle_download(self, value):
        return self.set_bool(SETTINGS.SUBTITLE_DOWNLOAD, value, echo_level=2)
    ...
    def get_thumbnail_size(self, value=None):
        default = 1
        if value is None:
            value = self.get_int(SETTINGS.THUMB_SIZE, default, echo_level=2)
    ...
    def safe_search(self):
        index = self.get_int(SETTINGS.SAFE_SEARCH, 0, echo_level=2)
        return self._SAFE_SEARCH_LEVELS[index]

    def age_gate(self):
        return self.get_bool(SETTINGS.AGE_GATE, True, echo_level=2)

    def verify_ssl(self, value=None):
        if value is not None:
            return self.set_bool(SETTINGS.VERIFY_SSL, value, echo_level=2)

        if sys.version_info <= (2, 7, 9):
            verify = False
        else:
            verify = self.get_bool(SETTINGS.VERIFY_SSL, True, echo_level=2)
        return verify

    def requests_timeout(self, value=None):
        if value is not None:
            self.set_int(SETTINGS.CONNECT_TIMEOUT, value[0], echo_level=2)
            self.set_int(SETTINGS.READ_TIMEOUT, value[1], echo_level=2)
            return value

        connect_timeout = self.get_int(SETTINGS.CONNECT_TIMEOUT,
                                       9,
                                       echo_level=2) + 0.5
        read_timout = self.get_int(SETTINGS.READ_TIMEOUT, 27, echo_level=2)
        ...
        if value is not None:
            for setting_name, setting in value.items():
                ...
                if setting_type is int:
                    self.set_int(setting_name, setting_value, echo_level=2)
                elif setting_type is str:
                    self.set_string(setting_name, setting_value, echo_level=2)
                else:
                    self.set_bool(setting_name, setting_value, echo_level=2)
            return value

        proxy_source = self.get_int(SETTINGS.PROXY_SOURCE, 1, echo_level=2)
        if not proxy_source:
            return None
    ...
    def allow_dev_keys(self):
        return self.get_bool(SETTINGS.ALLOW_DEV_KEYS, False, echo_level=2)

    def use_mpd_videos(self, value=None):
        if self.use_isa():
            if value is not None:
                return self.set_bool(SETTINGS.MPD_VIDEOS, value, echo_level=2)
            return self.get_bool(SETTINGS.MPD_VIDEOS, True, echo_level=2)
        return False
    ...
    def live_stream_type(self, value=None):
        ...
        if value is not None:
            return self.set_int(setting, value, echo_level=2)
        value = self.get_int(setting, default, echo_level=2)
    ...
    def httpd_port(self, value=None):
        ...
        if value is None:
            port = self.get_int(SETTINGS.HTTPD_PORT, default, echo_level=2)
        else:
            port = value
        ...
        if value is not None:
            return self.set_int(SETTINGS.HTTPD_PORT, port, echo_level=2)
        return port
    ...
    def httpd_whitelist(self):
        whitelist = self.get_string(SETTINGS.HTTPD_WHITELIST, '', echo_level=2)
    ...
    def httpd_sleep_allowed(self, value=None):
        if value is not None:
            return self.set_bool(SETTINGS.HTTPD_IDLE_SLEEP, value, echo_level=2)
        return self.get_bool(SETTINGS.HTTPD_IDLE_SLEEP, True, echo_level=2)
    ...
    def api_config_page(self):
        return self.get_bool(SETTINGS.API_CONFIG_PAGE, False, echo_level=2)

    def api_id(self, new_id=None):
        if new_id is not None:
            self.set_string(SETTINGS.API_ID, new_id, echo_level=2)
            return new_id
        return self.get_string(SETTINGS.API_ID, echo_level=2)

    def api_key(self, new_key=None):
        if new_key is not None:
            self.set_string(SETTINGS.API_KEY, new_key, echo_level=2)
            return new_key
        return self.get_string(SETTINGS.API_KEY, echo_level=2)

    def api_secret(self, new_secret=None):
        if new_secret is not None:
            self.set_string(SETTINGS.API_SECRET, new_secret, echo_level=2)
            return new_secret
        return self.get_string(SETTINGS.API_SECRET, echo_level=2)

    def get_location(self):
        location = self.get_string(SETTINGS.LOCATION,
                                   '',
                                   echo_level=2).replace(' ', '').strip()
    ...
    def set_location(self, value):
        self.set_string(SETTINGS.LOCATION, value, echo_level=2)

    def get_location_radius(self):
        return ''.join((
            self.get_int(SETTINGS.LOCATION_RADIUS, 500, str, echo_level=2),
            'km'
        ))

    def get_play_count_min_percent(self):
        return self.get_int(SETTINGS.PLAY_COUNT_MIN_PERCENT, 0, echo_level=2)

    def use_local_history(self):
        return self.get_bool(SETTINGS.USE_LOCAL_HISTORY, False, echo_level=2)

    def use_remote_history(self):
        return self.get_bool(SETTINGS.USE_REMOTE_HISTORY, False, echo_level=2)
    ...
    def mpd_video_qualities(self, value=None):
        ...
        if not self.use_mpd_videos():
            return []
        value = self.get_int(SETTINGS.MPD_QUALITY_SELECTION, 4, echo_level=2)
        return [quality
    ...
    def item_filter(self, update=None, override=None, exclude=None):
        if override is None:
            override = self.get_string_list(SETTINGS.HIDE_VIDEOS, echo_level=2)
    ...
    def subscriptions_filter(self, value=None):
        ...
        return self.get_string(SETTINGS.SUBSCRIPTIONS_FILTER_LIST,
                               '',
                               echo_level=2).replace(', ', ',')

    def shorts_duration(self, value=None):
        if value is not None:
            return self.set_int(SETTINGS.SHORTS_DURATION, value, echo_level=2)
        return self.get_int(SETTINGS.SHORTS_DURATION, 60, echo_level=2)
    ...
    def show_detailed_description(self, value=None):
        if value is not None:
            return self.set_bool(SETTINGS.DETAILED_DESCRIPTION,
                                 value,
                                 echo_level=2)
        return self.get_bool(SETTINGS.DETAILED_DESCRIPTION, True, echo_level=2)

    def show_detailed_labels(self, value=None):
        if value is not None:
            return self.set_bool(SETTINGS.DETAILED_LABELS, value, echo_level=2)
        return self.get_bool(SETTINGS.DETAILED_LABELS, True, echo_level=2)

    def get_language(self):
        return self.get_string(SETTINGS.LANGUAGE,
                               'en_US',
                               echo_level=2).replace('_', '-')

    def set_language(self, language_id):
        return self.set_string(SETTINGS.LANGUAGE, language_id, echo_level=2)

    def get_region(self):
        return self.get_string(SETTINGS.REGION, 'US', echo_level=2)

    def set_region(self, region_id):
        return self.set_string(SETTINGS.REGION, region_id, echo_level=2)
    ...
    def set_history_playlist(self, value):
        return self.set_string(SETTINGS.HISTORY_PLAYLIST, value, echo_level=2)
    ...
    if current_system_version.compatible(20):
        ...
        def get_label_color(self, label_part):
            ...
            return self.get_string(setting_name, 'white', echo_level=2)
    else:
        _COLOR_MAP = {
            'commentCount': 'cyan',
            'favoriteCount': 'gold',
    ...
    def log_level(self, value=None):
        if value is not None:
            return self.set_int(SETTINGS.LOG_LEVEL, value, echo_level=2)
        return (self.get_int(SETTINGS.LOG_LEVEL, 0, echo_level=2)
