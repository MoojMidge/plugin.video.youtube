...
def get_connect_address(context, as_netloc=False, address=None):
    ...
    try:
        ...
        logging.exception(('Failed to create socket', 'Exception: %r'), exc)
        ...
        try:
            ...
            logging.exception(('Failed to connect', 'Exception: %r'), exc)
            connect_address = xbmc.getIPAddress()
        else:
            try:
                connect_address = sock.getsockname()[0]
            except socket.error as exc:
                logging.exception(('No address', 'Exception: %r'), exc)
                connect_address = xbmc.getIPAddress()
        finally:
            sock.close()
