...
from traceback import extract_stack, print_exception, print_list
        ...
        while tb_obj:
            ...
            if next_tb_obj:
                ...
                with StringIO() as output:
                    ...
                    print_list(extract_stack(tb_obj.tb_frame)[:-1], file=output)
...
class KodiLogger(logging.Logger):
    ...
    def findCaller(self, stack_info=False, stacklevel=1):
        ...
        if stack_info:
            with StringIO() as output:
                output.write('Stack (most recent call last):\n')
                print_list(extract_stack(last_frame)[:-1], file=output)
                stack_info = output.getvalue()
                if stack_info[-1] == '\n':
                    stack_info = stack_info[:-1]
        else:
