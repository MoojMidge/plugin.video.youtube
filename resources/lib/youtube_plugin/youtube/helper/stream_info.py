...
class StreamInfo(YouTubeRequestClient):
    ...
    def _error_hook(**kwargs):
        ...
        if not json_data or 'error' not in json_data:
            info = ('Request - Failed'
                    ...
                    '\n\tClient:   |{client}|'
                    '\n\tAuth:     |{auth}|')
            return None, info, kwargs, data, exception
    ...
    def _get_player_key(self, html):
        ...
        if start_index != -1:
            ...
            logging.debug('Player key found: {0}'.format(player_key))
            return player_key
        return None
    ...
    def _process_signature_cipher(self, stream_map):
        if self._cipher is None:
            logging.debug('signatureCipher detected')
        ...
        if not signature:
            try:
                ...
                logging.exception(('Failed to extract URL',
                                   'Exception: {exc!r}',
                                   'Signature: |{sig}|'),
                                  exc=exc,
    ...
    def _process_url_params(self,
        ...
        if self._calculate_n and 'n' in params:
            ...
            if new_n:
                ...
                logging.error('nsig handling failed')
    ...
    def _process_adaptive_streams(self,
        ...
        if not video_data and not audio_only:
            logging.debug('Generate MPD: No video mime-types found')
            return None, None
    ...
    def _generate_mpd_manifest(self,
        ...
        if not self.BASE_PATH:
            logging.error('Unable to access temp directory', stack_info=True)
            return None, None
